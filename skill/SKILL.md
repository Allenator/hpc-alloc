...
- Multi-cluster: config may define several `[cluster.X]` entries, but only Bouchet is live-validated (multi-cluster paths are tested offline only). If a secondary cluster is unreachable, commands degrade with `note: cluster '<x>' is unreachable — skipping it` and its allocations show as `UNKNOWN` — report that to the user rather than treating it as an error.
