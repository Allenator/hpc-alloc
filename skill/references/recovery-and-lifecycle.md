...
If interruption rolls back the local reservation before its transaction commits, emit no recovery guidance because no operation exists and the remote submit was never entered. If an interrupt lands while the reservation itself is still running, so that its commit may have just landed but remote dispatch was never entered, expect the job closed locally as `submit-failed` with no operation left to recover, and conservative recovery guidance only when that close cannot be confirmed. Treat the window as that narrow: everything from the remote call onward counts as possibly overlapping the mutation, so an interrupt during the SSH setup inside it is already the ambiguous case, not this one. If interruption or transport loss occurs after dispatch may have begun, preserve the normal interrupt or error result, print `do not resubmit`, and use the exact recovery command; include a trusted job ID when the remote acknowledgement arrived but the local write failed. If even the ambiguity update cannot be recorded, the `PREPARED` operation stays unresolved and remains recoverable by the printed ID.
