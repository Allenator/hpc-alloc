...
Treat each submit or cancellation as a prepare, one-shot remote call, and acknowledgement protocol using short local transactions. Let an operation-scoped advisory lock serialize its live owner with recovery and abandonment; if that lock is active, fail fast and retry only after the owning process exits.
...
Interpret `CANCEL_PENDING` as proof that the guarded remote cancellation was not dispatched, because `AMBIGUOUS` is committed immediately before the one-shot call. Interpret `AMBIGUOUS` as may-have-run and block another cancellation attempt until recovery or explicit abandonment resolves that intent.

On interruption before the cancellation reservation commits, emit no recovery guidance. On interruption with durable `CANCEL_PENDING`, close the undispatched intent locally when possible; if that closure cannot be confirmed, print its exact recovery command. On interruption after phase becomes `AMBIGUOUS`, preserve 130 and print the exact recovery command.
