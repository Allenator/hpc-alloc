...
| `hpc-alloc logs NAME\|JOBID\|@OPERATION [--cluster C] [-n/--lines LINES] [-f/--follow]` | Read or follow an operation-scoped managed log; `-n` defaults to 100 lines. Wait safely for start with `-f`; never make follow imply cancellation. |
...
- Read each `jobs` entry through `selector`, `operation_id`, `jobid`, `cluster`, `name`, `kind`, `phase`, `scheduler_state`, `evidence_detail`, `ever_started`, `current_node`, `last_node`, `terminal_state`, `exit_code`, `final_source`, `partition`, `time`, `gpus`, and `alias`. Keep a job finalized during the pass in `jobs` once rather than repeating it in `discovered`.
- Read a `jobs` or `why` `phase` as one of `QUEUED`, `ACTIVE`, `STARTED_INACTIVE`, `REQUEUEING`, `TERMINAL_CANDIDATE`, `FINAL`, `UNCERTAIN`, or `SUBMITTING`. Treat `UNCERTAIN` as a failed observation rather than a failed job. Treat `SUBMITTING` as a submission with no acknowledged Slurm job ID yet: a real job may already be running on the cluster, so never read it as "nothing was submitted" and never resubmit. Reconcile the printed `hpc-alloc recover OPERATION_ID` first.
- Read each `discovered` entry through `cluster`, `jobid`, `operation_id`, `selector`, `job_kind`, `classification`, `state`, `node`, `partition`, `time_left`, `owner`, and `name`; accept `untracked-owned`, `other-machine`, `unresolved-operation-match`, `duplicate-operation`, `local-final-conflict`, and `operation-identity-conflict` as classification values, and treat its canonical selector as evidence rather than mutation authority.
