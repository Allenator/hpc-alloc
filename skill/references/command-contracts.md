...
Apply the shared `up` and `run` resource flags `--cluster`, `-p/--partition`, `-t/--time`, `-c/--cpus`, `--mem`, `-G/--gpus`, `-C/--constraint`, and `--dry-run`. Resolve resource values in this order: CLI flag, selected `[cluster.NAME]`, `[defaults]`, then built-in fallback. When `-G TYPE:N` is given without `-p`, `up` and `run` refine the resolved default partition on the real submit path to one that actually offers that GPU type: the default is kept if it already offers the type, otherwise the single eligible dedicated partition offering it is auto-selected and announced (preemptible `scavenge*` and short `*devel` pools are excluded), and the command refuses locally when several qualify or none do; `--dry-run` cannot resolve topology offline, so it instead warns to pass `-p` (or run `avail --for -G TYPE:N`). Before dispatching, `up` and `run` refuse locally a resolved partition the account, QOS, or groups cannot use — so an access error is caught before submission rather than becoming an ambiguous mutation to recover; the check falls open (submits normally) when the access data is unavailable.
...
- Read `why --json` as one job assessment and diagnosis, `avail --json` as `{ "partitions": { ... } }`, `avail --for --json` as `{ "for": { resolved request }, "probes": [ { "partition", "schedulable", "start", "detail" } ] }` ordered soonest-first (with an added `error` string and empty `probes` when the requested GPU type is unknown or unavailable), and `partitions --json` as an array of partition objects, each carrying an `eligible` flag (true, false, or null when access data is unavailable).
