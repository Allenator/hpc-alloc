...
- Read `why --json` as one job assessment and diagnosis, `avail --json` as `{ "partitions": { ... } }` where each partition object carries an `eligible` flag (true, false, or null when access data is unavailable), `avail --for --json` as `{ "for": { resolved request }, "probes": [ { "partition", "schedulable", "start", "detail" } ] }` ordered soonest-first (with an added `error` string and empty `probes` when the requested GPU type is unknown or unavailable), and `partitions --json` as an array of partition objects, each carrying an `eligible` flag (true, false, or null when access data is unavailable).
