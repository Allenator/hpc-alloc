...
- **Terminal-free re-auth**: `hpc-alloc connect --push` authenticates via Duo push — an `SSH_ASKPASS` helper answers Duo's menu with "1" (send push), and you approve on your phone. No secret passes through the client, so agents can run it (after telling you to expect the push).
...
- **An UNTRACKED-allocation entry in `status`** — a running hpc-alloc job this machine doesn't track: a lost record, or an allocation created from your *other* machine (state is per-machine). Verify before reclaiming with `hpc-alloc cancel <jobid>`; entries marked "just submitted" are an `up` from another window and should be left alone.
- **Exit codes and streams**: `run` mirrors the job's exit code (non-COMPLETED ends are nonzero; it refuses to claim success without a final accounting record); `logs -f` always exits 0 after a clean stream; connection loss is exit 3; a closed output pipe is exit 141. All progress notices go to stderr — stdout of `--json` commands is pure JSON.
