...
- Interactive commands ride out drops (during `up`'s wait, it retries for up to 10 minutes and re-prompts Duo when the network returns). Non-interactive callers get **exit code 3** with instructions, because only a human can answer Duo: reconnect the VPN, run `hpc-alloc connect`, retry.
