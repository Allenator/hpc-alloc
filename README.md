...
Progress and recovery notices go to stderr. JSON stdout remains machine-only. Exit 3 is contextual rather than globally reserved: typed authentication, host-key, and transport failures use it, and a possibly dispatched cancellation may surface its recovery guidance through a transport-class failure. A foreground batch command may also itself exit 3, while `ssh` and `sync` can return delegated OpenSSH or rsync statuses. Interpret exit 3 together with the invoked command and stderr; configuration, scheduler, protocol, and other hpc-alloc application failures normally use exit 1.
