...
| `avail [--cluster NAME] [-p PARTITION] [--json]` | Summarize idle CPUs and free GPUs for one cluster (idle GRES is not a schedulability guarantee). With `--for` plus a request (`-G/-c/--mem/-t`), probe where that request would start soonest across the eligible partitions — a scheduler dry-run that submits no job, ranked by advisory estimated start. |
...
When you request a specific GPU type with `-G TYPE:N` but omit `-p/--partition`, `up` and `run` select the partition on the real submit path: if the default partition already offers that type it is kept, otherwise the tool auto-selects the single eligible dedicated partition that offers it (skipping preemptible `scavenge*` and short `*devel` pools) and prints which it chose, and it refuses locally — rather than dispatching an unschedulable request — when several partitions qualify or none do. A `--dry-run` cannot reach the cluster to resolve this, so it instead warns you to pass `-p` (or consult `avail --for -G TYPE:N`); selection also falls open to the configured default if the availability data cannot be read.
...
- `avail --json` returns `{ "partitions": { ... } }`; `avail --for --json` returns `{ "for": { resolved request }, "probes": [ { "partition", "schedulable", "start", "detail" } ] }`, ordered soonest-first, and instead carries an `error` string with empty `probes` when the requested `-G TYPE:N` names a GPU type no partition offers.
