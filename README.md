...
| `avail [--cluster NAME] [-p PARTITION] [--json]` | Summarize idle CPUs and free GPUs for one cluster (idle GRES is not a schedulability guarantee), marking each partition with whether your account may submit to it (`ELIGIBLE` column; `eligible` in `--json`). With `--for` plus a request (`-G/-c/--mem/-t`), probe where that request would start soonest across the eligible partitions — a scheduler dry-run that submits no job, ranked by advisory estimated start. |
